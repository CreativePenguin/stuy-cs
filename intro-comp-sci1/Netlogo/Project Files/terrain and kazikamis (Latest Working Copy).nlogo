...
turtles-own [health opposite-color mode tick-counter]
...
to go
  ...
  ask turtles with [not (breed = bullets)] [
...
to place-musket [x]
    ...
    if click? = 0 [
      if x = red [
        create-muskets 1 [ redspawn health-value]
       ]
      if x = blue [
        create-muskets 1 [bluespawn health-value]
      ]
    ]
  ]
  [set click? 0]
...
to place-pawn [x]
   ifelse mouse-inside? and mouse-down? [
    if click? = 0 [
      if x = red [
        create-pawns 1 [redspawn health-value]
       ]
      if x = blue [
        create-pawns 1 [bluespawn health-value]
      ]
    ]
  ]
  [set click? 0]
...
to place-kamikazi [x]
  ifelse mouse-inside? and mouse-down?  [
    if click? = 0 [
      if x = red [
        create-kamikazis 1 [redspawn health-value]
       ]
      if x = blue [
        create-kamikazis 1 [bluespawn health-value]
      ]
    ]
  ]
  [set click? 0]
...
to place-ninja [x]
  ifelse mouse-inside? and mouse-down?  [
    if click? = 0 [
      if x = red [
        create-ninjas 1 [redspawn health-value]
       ]
      if x = blue [
        create-ninjas 1 [bluespawn health-value]
      ]
    ]
  ]
  [set click? 0]
...
to heal-move
  ask turtles with [color = [color] of self] [
...
to hitbullet
  if any? other turtles-here with [not (color = [team] of myself)] [
    ask turtles-here [set health health - (90 + random 40)]
    die
  ]
end
...
to kms
  if any? turtles in-radius 1 with [color = [opposite-color] of myself][
    ask patch-here [
      sprout 1 [set breed explosions]
    ]
    ask turtles in-radius 3 with [not (breed = explosions)] [die]
  ]
end
...
to around-wall
  if any? patches in-radius vision-range with [pcolor = orange] and not mode = "attack wall" [
    ask myself [
...
to around-trench
  if any? patches in-radius vision-range with [pcolor = brown] and not mode = "attack wall"[
    ask myself [
      set mode "to trench"
    ]
  ]
...
;;Variables we need to account for: (for each troop)
;Health
;Movement Speed
;Attack Speed
;Attack Damage
...
troop-formation
troop-formation
"basic" "v formation" "total mayhem"
2
...
Warning: Running the total mayhem formation might crash netlogo
