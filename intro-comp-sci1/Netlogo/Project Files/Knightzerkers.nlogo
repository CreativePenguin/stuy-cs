...
breed [berserks berserker]
breed [walls wall]
turtles-own [health  speed   opposite-color   hit-chance kills]
...
to go
  ...
  repeat time-between-fire [
    ...
    ask medics [medicstuff]
    ask knights [pawnstuff]
    ask berserks [berserkstuff]
...
to die?
  if health <= 0 [
    ask turtles in-radius 5 [ set kills kills + 1]
    die
  ]
end
...
to berserkstuff
  if any? turtles with [color = [opposite-color] of myself][berserkattack]
  melee-move
end
...
to berserkattack
  ask turtles in-radius 0.5 with [color = [opposite-color] of myself] [
    set health health - (30 + (30 * kills))
    set speed speed + (0.2 * kills)
    bk 2
  ]
end
...
to-report health-value
  ...
  if breed = knights [report 250]
  if breed = berserks [report 150]
end
...
to-report movement-value
  ...
  if breed = knights [report 0.2]
  if breed = berserks [report 0.2 + (0.2 * kills)]
end
...
to place-red
  ...
  if troop-type = "berserkers" [place-berserker red]
end
...
to place-blue
  ...
  if troop-type = "knights" [place-knight blue]
  if troop-type = "berserkers" [place-berserker blue]
end
...
to place-knight [x]
   ifelse mouse-inside? and mouse-down? [
    if click? = 0 [
      if x = red [
        create-knights 1 [redspawn 250]
       ]
      if x = blue [
        create-knights 1 [bluespawn 250]
      ]
    ]
  ]
  [set click? 0]
...
to place-berserker [x]
   ifelse mouse-inside? and mouse-down? [
    if click? = 0 [
      if x = red [
        create-berserks 1 [redspawn 150]
       ]
      if x = blue [
        create-berserks 1 [bluespawn 150]
      ]
    ]
  ]
  [set click? 0]
...
to formationspawn [col bred spd hed hp]
  sprout 1 [
    set color col
    set breed bred
    set speed spd
...
to place-troop-formation-red
  ...
  ask n-of 100 patches with [pxcor < 0][
      formationspawn (red) (kamikazis) (0.2) (90) 25
    ]
    formationspawn2
  ]
  if troop-formation = "british army" [
    ask patches with [pxcor < 0 and remainder pxcor 10 = 0][
      formationspawn (red) (muskets) (0.2) (90) 75
    ]
    formationspawn2
  ]
  if troop-formation = "one v one" [
    ask patch -10 0 [
      formationspawn (red) (pawns) (0.2) (90) 100
    ]
  ]
end
...
to place-troop-formation-blue
  if troop-formation = "total mayhem" [
  ask n-of 100 patches with [pxcor > 0][
      formationspawn (blue) (kamikazis) (0.2) (270) 25
    ]
    formationspawn2
  ]
  if troop-formation = "british army" [
    ask patches with [pxcor > 0 and remainder pxcor 10 = 0][
      formationspawn (blue) (muskets) (0.2) (270) 75
    ]
    formationspawn2
  ]
  if troop-formation = "one v one" [
    ask patch 10 0 [
      formationspawn (blue) (pawns) (0.2) (270) 100
    ]
  ]
end
...
troop-type
troop-type
"muskets" "pawns" "kamikazis" "ninjas" "knights" "berserkers" "medics"
1
...
troop-formation
troop-formation
"test" "british army" "total mayhem" "one v one"
3
...
After clicking the setup button, you can use the choosers to place down terrain and preset troop formations. Clicking the place-red and place-blue will allow you to place down troops on their respective sides and the place-terrain will allow you to place down grass, trenches, and walls. 
...
Mark De Paula and Winston Peng
@#$#@#$#@
default
true
0
