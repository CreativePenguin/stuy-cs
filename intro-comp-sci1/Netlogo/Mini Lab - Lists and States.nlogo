turtles-own [ meetings mode spinTimer]
...
to setup
  ...
  [
    ...
    [
      set size 2
      set mode one-of ["forward" "wiggle" "spinning"]
      set meetings []
    ]
  ]
end
...
to go
  ask turtles[
    ...
    if mode = "forward" [advance]
    if mode = "wiggle" [wiggle]
    if mode = "spinning" [spin]
    meets
  ]
  tick
end
...
to chance-spin
  if random 200 = 1 [set mode "spinning"]
end
...
to meets
  ...
  [
    ...
    [
      set meetings lput [who] of myself meetings
    ...
    ;;  Both turtles add each other's who number to the meetings list
    ;;  Both turtles exchange states. (do this once! not twice)
@#$#@#$#@
GRAPHICS-WINDOW
210
10
